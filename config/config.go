...
func LoadConfig(path string) IConfig {
	...
	return &config{
		app: &app{
			...
			writeTime: func() time.Duration {
				w, err := strconv.Atoi(envMap["APP_WRITE_TIMEOUT"])
				if err != nil {
					log.Fatalf("load write failed: %v", err)
				}
		...
		db: &db{
			...
			port: func() int {
				...
				if err != nil {
					log.Fatalf("load dbport failed: %v", err)
				}
				return p
			}(),
...
func (a *app) WriteTimeout() time.Duration { return a.writeTime }
...
func (d *db) MaxConnection() int { return d.maxConnections }
