...
// สำหรับทำ Sign-in
type UserCredentailCheck struct {
	Id       string `db:"id"`
...
func (obj *UserRegisterReq) BcryptHashing() error {
	...
	if err != nil {
		return fmt.Errorf("hashes password failed: %v", err)
	}
...
// การสร้าง UserClaims ไม่ควรส้รางข้อมูลที่เป็น Secret เช่น password เอาแค่ข้อมูลพื้นฐาน
...
type UserRefreshCredential struct {
	RefreshToken string `json:"refreash_token" form:"refreash_token"`
}
